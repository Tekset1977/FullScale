...
#include "esp_sleep.h"      // esp_sleep_enable_gpio_wakeup()
...
static const uint8_t  PIN_BARO_INT = 25U;  // MPL3115A2 INT1 → wire to this GPIO
...
static const float    CFG_ALT_WAKE_THRESHOLD_M    = 1.0f;
...
static bool initBarometer(void) {
    ...
    for (int retry = 0; retry < CFG_MAX_RETRIES; retry++) {  // R2: bounded
        uint8_t who = 0U;
        if (i2cReadBytes(I2C_ADDR_BARO, 0x0CU, &who, 1U) != 1U || who != 0xC4U) {
            delay(100); continue;
        }
        // BAR_IN = sea-level pressure in Pa/2 (101326/2 = 50663 = 0xC5E7)
        if (!i2cWriteReg(I2C_ADDR_BARO, 0x14U, 0xC5U)) { delay(100); continue; }
        if (!i2cWriteReg(I2C_ADDR_BARO, 0x15U, 0xE7U)) { delay(100); continue; }
...
// Direct one-shot altitude read — triggers a conversion, polls DRDY, returns metres.
// Used only at setup (baseline) and sleep-arm (P_TGT anchor); not the hot path.
static float baroReadAltitudeBlocking(void) {
    // Standby, then one-shot: altimeter mode (0x80) | OST (0x02) | SBYB (0x01)
    i2cWriteReg(I2C_ADDR_BARO, 0x26U, 0x00U);
    delay(5);
    i2cWriteReg(I2C_ADDR_BARO, 0x26U, 0x83U);

    // Poll DR_STATUS bit 3 (PDRDY) — timeout after 200 ms
    uint32_t t0 = (uint32_t)millis();
    uint8_t  status = 0U;
    do {
        delay(5);
        i2cReadBytes(I2C_ADDR_BARO, 0x00U, &status, 1U);
    } while (!(status & 0x08U) && ((uint32_t)millis() - t0) < 200U);

    float alt = 0.0f, temp = 0.0f;
    readBaroRawBytes(&alt, &temp);   // already validated + decoded
    return alt;
}

//  R4/R5/R7: Configure MPL3115A2 altitude-change interrupt on INT1
//  Fires (active-low) when altitude shifts by more than window_m metres from
//  whatever the device last latched as its reference point.
//  Registers touched:
//    0x26 CTRL_REG1  — standby / active
//    0x19 P_WND_MSB  — altitude window integer part (1 LSB = 1 m)
//    0x1A P_WND_LSB  — altitude window fractional part (zeroed here)
//    0x28 CTRL_REG3  — INT1/INT2 polarity & pad type (active-low, open-drain)
//    0x29 CTRL_REG4  — interrupt enable: bit 2 = INT_EN_PCHG
//    0x2A CTRL_REG5  — interrupt routing: bit 2 = 1 routes PCHG to INT1
//  INT_EN_PCHG fires when |current_alt - P_TGT| > P_WND.
//  P_TGT must be loaded with the current altitude at arm time — it is NOT
//  a rolling delta. Without this, the chip compares against zero (boot garbage)
//  and the condition is permanently true or permanently false.
//  current_alt_m — absolute altitude now (P_TGT anchor)
//  window_m      — wake when altitude drifts this many metres from current_alt_m
static bool configureBaroAltInterrupt(float current_alt_m, float window_m) {
    if (!assertRange(current_alt_m, ALT_MIN_M, ALT_MAX_M, ERR_SENSOR_RANGE)) { return false; }
    if (window_m <= 0.0f || window_m > 255.0f) { return false; }
    if (I2C_ADDR_BARO == 0U) { return false; }

    // Step 1 — standby (registers only writable with SBYB=0)
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x26U, 0x00U)) { return false; }
    delay(10);

    // Step 2 — P_TGT = current altitude (signed int16, 1 LSB = 1 m)
    //          P_TGT_MSB (0x16) = bits[15:8],  P_TGT_CSB (0x17) = bits[7:0]
    int16_t tgt = (int16_t)current_alt_m;
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x16U, (uint8_t)(tgt >> 8)))   { return false; }
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x17U, (uint8_t)(tgt & 0xFF))) { return false; }

    // Step 3 — P_WND: integer metres, fractional zeroed
    uint8_t wnd = (uint8_t)constrain((int)window_m, 1, 255);
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x19U, wnd))   { return false; }
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x1AU, 0x00U)) { return false; }

    // Step 4 — CTRL_REG3: INT1 active-low, open-drain (bits 5:4 = 00)
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x28U, 0x00U)) { return false; }

    // Step 5 — CTRL_REG4: enable INT_EN_PCHG (bit 2)
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x29U, 0x04U)) { return false; }

    // Step 6 — CTRL_REG5: route PCHG to INT1 (bit 2 = 1)
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x2AU, 0x04U)) { return false; }

    // Step 7 — re-activate: altimeter mode (bit 7), OSR=0, SBYB=1 (bit 0)
    if (!i2cWriteReg(I2C_ADDR_BARO, 0x26U, 0x81U)) { return false; }
    delay(20);

    Serial.printf("Baro INT armed — P_TGT=%d m  P_WND=%d m  GPIO%u\n",
                  (int)tgt, (int)wnd, PIN_BARO_INT);
    return true;
}

//  R4/R7: Clear pending MPL3115A2 interrupt by reading INT_SOURCE (0x12) then
//  draining the output data registers so the next conversion starts clean.
static void clearBaroInterrupt(void) {
    uint8_t int_src = 0U;
    (void)i2cReadBytes(I2C_ADDR_BARO, 0x12U, &int_src, 1U);   // clears INT_SOURCE
    uint8_t buf[5] = {0U};
    (void)i2cReadBytes(I2C_ADDR_BARO, 0x01U, buf, 5U);         // drains OUT_P / OUT_T
    Serial.printf("Baro INT_SOURCE cleared (was 0x%02X)\n", int_src);
}

//  R4/R7: Enter light sleep — wake exclusively on MPL3115A2 altitude-change interrupt.
//  No timer fallback: if the barometer is unavailable or arming fails, sleep is
//  skipped rather than sleeping without a wakeup source (which would hang forever).
static void enterLightSleep(void) {
    ...
    if (!g_mpl_ok) {
        Serial.println("enterLightSleep: barometer unavailable — sleep skipped");
        wakeReopenLogFile();
        return;
    }

    float current_alt_m = baroReadAltitudeBlocking();
    if (!configureBaroAltInterrupt(current_alt_m, CFG_ALT_WAKE_THRESHOLD_M)) {
        Serial.println("enterLightSleep: baro interrupt arm failed — sleep skipped");
        wakeReopenLogFile();
        return;
    }

    if (digitalRead(PIN_BARO_INT) == LOW) {
        Serial.println("enterLightSleep: INT1 already low — clearing, skipping sleep");
        clearBaroInterrupt();
    ...
    gpio_wakeup_enable((gpio_num_t)PIN_BARO_INT, GPIO_INTR_LOW_LEVEL);
    esp_sleep_enable_gpio_wakeup();

    Serial.printf("Sleeping — P_TGT=%.1f m  wake window=%.1f m  GPIO%u\n",
                  current_alt_m, CFG_ALT_WAKE_THRESHOLD_M, PIN_BARO_INT);
    Serial.flush();
    esp_light_sleep_start();

    esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_GPIO);
    gpio_wakeup_disable((gpio_num_t)PIN_BARO_INT);
    clearBaroInterrupt();

    Serial.println("woke up — baro altitude-change interrupt");
    s_wake_time_ms = (unsigned long)millis();
...
void setup(void) {
    ...
    // R5: Assert baro interrupt pin is high (not asserted) at boot
    pinMode(PIN_BARO_INT, INPUT_PULLUP);
    ...
    g_alt_baseline_m = baroReadAltitudeBlocking();
