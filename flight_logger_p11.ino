...
static const float    ALT_BAND_LO_M = 200.0f * 0.3048f;   // write first value as actual foot value
...
// Per-sensor sample structs — one per ring buffer
struct IcmSample {
    unsigned long timestamp;
    float ax, ay, az;
    float gx, gy, gz;
...
// -----------------------------------------------------------------------------
//  Ring buffer sizes  (tune to match worst-case SD write latency)
...
static IcmRingBuf g_icm_buf = {{}, 0U, 0U, 0U};
static MplRingBuf g_mpl_buf = {{}, 0U, 0U, 0U};
...
// =============================================================================
//  Ring buffer push / pop helpers
// =============================================================================

// ICM push — overwrites oldest entry and warns on overrun (non-fatal)
static void icmBufPush(const IcmSample* s) {
...
// ICM pop — returns false when buffer is empty
static bool icmBufPop(IcmSample* s) {
...
// MPL push — same overwrite-on-full policy as ICM push
static void mplBufPush(const MplSample* s) {
    if (!assertNotNull(s, ERR_SENSOR_RANGE)) { return; }
...
// MPL pop — returns false when buffer is empty
static bool mplBufPop(MplSample* s) {
    if (!assertNotNull(s, ERR_SENSOR_RANGE)) { return false; }
...
//  R4/R5: Drain both ring buffers and write paired rows to the SD log.
//  Pops one IcmSample and one MplSample per iteration and assembles them
//  into a SensorData row.  Stops when either buffer runs dry so the two
//  streams stay in lock-step.
static void drainBufsToLog(void) {
    // R5: Assert file is open before touching it
    ...
    while (g_icm_buf.count > 0U && g_mpl_buf.count > 0U) {
        ...
        if (!icmBufPop(&icm)) { break; }   // R7: pop return checked
        if (!mplBufPop(&mpl)) { break; }   // R7: pop return checked

        // Assemble the combined row — use ICM timestamp as the row timestamp
        SensorData d;
        ...
        bool ok = writeLogData(&d);   // R7: return checked
#ifdef DEBUG
...
void loop(void) {
    ...
    // Push ICM fields into ICM ring buffer
    IcmSample icm_s = {
        data.timestamp,
        data.ax, data.ay, data.az,
        data.gx, data.gy, data.gz,
    ...
    // Push MPL fields into MPL ring buffer
    MplSample mpl_s = {
        data.timestamp,
        data.altitude,
        data.temperature
    ...
    // Drain both ring buffers to the SD log
    drainBufsToLog();
